...
class AdminProcess
{
   ...
   function AdminProcess(){
      ...
      /*Admin submitted approve user form */
      else if(isset($_POST['appuserlevel'])){
         $this->procApproveUser();
      }
   ...
   /**
    * procApproveUser - If the username is correct,
    * approve the user in the database.
    */
   function procApproveUser(){
	   global $session, $database, $form;
      /* Username error checking */
      $subuser = $this->checkUsername("appusername");
      ...
      if($form->num_errors > 0){
         ...
         $database->updateUserField($subuser, "approved", 1);
         header("Location: ".$session->referrer);
      }
   }
   /**
   ...
   function procDeleteUser(){
      ...
      /* Delete user from database */
      	 ...
      	 $q = "SELECT userid FROM ".TBL_USERS." WHERE username = '$subuser'";
         ...
         $q = "DELETE FROM ".TBL_PROFILE." WHERE userid = '$userid'";
         $database->query($q);
