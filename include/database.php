...
class MySQLDB
{
  ...
  /**
	* usernameApproved - Returns true if the username
	* has bee approved by an administrator
	*/
	function usernameApproved($username){
	      if(!get_magic_quotes_gpc()){
	         //$username = addslashes($username);
	         $username = mysql_real_escape_string($username);
	      }
	      $q = "SELECT approved FROM ".TBL_USERS." WHERE username = '$username'";
	      $result = mysql_query($q, $this->connection);
   ...
   /**
    * addNewFeedback - Inserts the given feedback
    * info into the database.
    * Returns true on success, false otherwise.
    */
   function addNewFeedback($username, $vidused, $dates, $goal, $location, $audience, 
   			$time, $length, $vidrole, $vidhelpful, $usedyvonne, $yvonneuseful, $continue){


		$vidused = mysql_real_escape_string($vidused);
		$dates = mysql_real_escape_string($dates);
		$goal = mysql_real_escape_string($goal);
		$location = mysql_real_escape_string($location);
		$audience = mysql_real_escape_string($audience);
		$time = mysql_real_escape_string($time);
		$length = mysql_real_escape_string($length);
		$vidrole = mysql_real_escape_string($vidrole);
		$vidhelpful = mysql_real_escape_string($vidhelpful);
		$yvonneuseful = mysql_real_escape_string($yvonneuseful);
		$continue = mysql_real_escape_string($continue);
		...
		if($usedyvonne){
			$q = "INSERT INTO ".TBL_FEEDBACK." VALUES ( 'NULL', '$userid', NOW(), '$vidused', '$dates', '$goal', '$location',". 
				"'$audience', '$time', '$length', '$vidrole', '$vidhelpful', 1, '$yvonneuseful', '$continue')";
		}
		else{
			$q = "INSERT INTO ".TBL_FEEDBACK." VALUES ( 'NULL', '$userid', NOW(), '$vidused', '$dates', '$goal', '$location',". 
				"'$audience', '$time', '$length', '$vidrole', '$vidhelpful', 0, 'NULL', '$continue')";
		}
    ...
    * parameter, in the user's row of the user table.
    */
   ...
   /**
    * updateProfileField - Updates a field, specified by the field
    * parameter, in the user's row of the profile table.
    */
   function updateProfileField($username, $field, $value){
   	  $q = "SELECT userid FROM ".TBL_USERS." WHERE username = '$username'";
      $result = mysql_query($q, $this->connection);
      ...
      $q = "UPDATE ".TBL_PROFILE." SET ".$field." = '$value' WHERE userid = '$userid'";
      return mysql_query($q, $this->connection);
   }
   ...
   /**
    * incrementViewedCount - Increases the viewed count of the file specified
    * by 1. Also creates an entry for the file if it doesn't exist.
   */
   function incrementViewedCount($filename){
   	  $q = "SELECT * FROM ".TBL_VIDEOS." WHERE filename = '$filename'";
      $result = mysql_query($q, $this->connection);
      	
      if (mysql_num_rows($result) == 0){
      	//query of filename failed, need to add it to the table
      	$q = "INSERT INTO ".TBL_VIDEOS." VALUES ( '$filename', '1')";
      	return mysql_query($q, $this->connection);
      }
      else{
      	$q = "UPDATE ".TBL_VIDEOS." SET hits = hits+1 WHERE filename = '$filename'";
      	return mysql_query($q, $this->connection);
      }
   }
