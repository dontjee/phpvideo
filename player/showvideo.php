<?
	...
	*/
	...
	//needed to verify login and decode the url
	...
	if($session->logged_in){
		...
		if(isset($_GET[XMOOV_GET_FILE]) && isset($_GET[XMOOV_GET_POSITION]))
		{
			...
			# get file name
			$fileName = htmlspecialchars(decode( str_replace( ".flv", "", $_GET[XMOOV_GET_FILE]) ) );		
			...
			if (!file_exists($file))
			{
				trigger_error('ERROR: xmoov-php could not find (' . $fileName . ').'); 
				exit();
			}
			
			#increment the watched count in the database
			$session->viewedVideo($fileName);
			...
			trigger_error("Error getting file, or position");
